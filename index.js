...
const { groupResponse_Welcome, groupResponse_Remove, groupResponse_Promote, groupResponse_Demote } = require('./myfunc.js')
const color = (text, color) => { return !color ? chalk.green(text) : chalk.keyword(color)(text) }
const store = makeInMemoryStore({ logger: pino().child({ level: 'silent', stream: 'store' }) })

const connectToWhatsApp = () => {
const client = makeWASocket({ logger: pino ({ level: 'silent' }), printQRInTerminal: true, auth: state, browser: ["SC BY Ziro", "Dekstop", "3.0"]})
console.log(color('[ Base Ori Luxxy ]\n', 'red'),color('\nInfo Script :\n➸ Baileys : Multi Device\n➸ Nama Script : Luxxy-MD\n➸ Creator : Luxxy\n\nFollow My Social Media Account All Yes :\n➸ My Youtube : LuxxyXD`\n\nThanks\n', 'red'))
...
groupResponse_Demote(client, update)
groupResponse_Promote(client, update)
groupResponse_Welcome(client, update)
groupResponse_Remove(client, update)
console.log(update)
})
